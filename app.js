...
crearBarra(C);
let LISP = document.getElementById("LISP");
crearBarra(LISP);
...
function efectoHabilidades(){
    ...
    if(distancia_skills>=300 && entro==false){
        ...
        const intervalC = setInterval(function(){
            pintarBarra(C, 14, 3, intervalC);
        },100);
        const intervalLISP = setInterval(function(){
            pintarBarra(LISP, 16, 4, intervalLISP);
        },100);
        const intervalLINUX = setInterval(function(){
            pintarBarra(LINUX, 9, 5, intervalLINUX);
        },100);
    }
}
